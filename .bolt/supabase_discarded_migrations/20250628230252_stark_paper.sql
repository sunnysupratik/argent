/*
  ...
  1. Drop Dependencies First
    ...
    - Drop views that reference user_id
    - Drop indexes on user_id columns

  2. Drop user_id Columns
    - Remove user_id from all 5 tables where it was recently added
    - Keep the original structure intact

  3. Recreate Necessary Policies
    - Recreate policies using the original column references
    - Ensure RLS continues to work properly
*/
...
BEGIN
  ...
  FOR table_name IN VALUES ('custom_users'), ('profiles'), ('accounts'), ('categories'), ('transactions') LOOP
    ...
    IF column_exists THEN
      RAISE NOTICE 'Table %: user_id column still exists (removal may have failed) - % records', table_name, record_count;
    ELSE
      RAISE NOTICE 'Table %: user_id column successfully removed - % records remain', table_name, record_count;
    END IF;
  END LOOP;
  ...
  FOR table_name IN VALUES ('custom_users'), ('profiles'), ('accounts'), ('categories'), ('transactions') LOOP
    ...
    IF column_exists THEN
      RAISE NOTICE 'Table %: user_name column present ✓', table_name;
    ELSE
      RAISE NOTICE 'Table %: user_name column missing ✗', table_name;
    END IF;
  END LOOP;
